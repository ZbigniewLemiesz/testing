...
public class NumberOfDiscIntersections {
    public int solution(int[] a) {
        int numberIntersectingPairs = 0;
        for (int i = 0; i <= a.length - 2; i++) {
            if (a[i] < a.length - 1 - i) {
                numberIntersectingPairs += a[i];
                int iRange = a[i] < Integer.MAX_VALUE - i ? a[i] + i : Integer.MAX_VALUE;
                for (int j = a[i] + i + 1; j <= a.length - 1; j++) {
                    int jRange = (j >= a[j]) ? j - a[j] : 0;
                    boolean isIntersect = (jRange <= iRange);
                    if (jRange <= iRange) {
                        numberIntersectingPairs++;
                    }
                }

            } else {
                numberIntersectingPairs += a.length - 1 - i;
            }
            if (numberIntersectingPairs > 10_000_000) {
                return -1;
            }
        }


        return numberIntersectingPairs;
    }
